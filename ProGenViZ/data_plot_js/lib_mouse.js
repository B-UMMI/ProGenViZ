...
var setup_mouse = function(plot_info) {
  ...
  //   the sidebar to show consistent colors and text
    ...
    var node_ib  = function(curr_node) {
      var curr_name   = curr_node.node.gene;
      var orig_name   = orig_link.source.node.gene;
    ...
    var node_im  = function(curr_node) {
      var curr_name   = curr_node.node.gene;
      var orig_name   = orig_link.target.node.gene;
    ...
    var src_name  = '<p>Gene: ' + orig_link.source.node.gene.split('...')[1] + '  Genome: ' + orig_link.source.node.gene.split('...')[0] + '</p>';
    var tgt_name  = '<p>Gene: ' + orig_link.target.node.gene.split('...')[1] + '  Genome: ' + orig_link.target.node.gene.split('...')[0] + '</p>';

    var html_ib   = on_mouseover_h('ib', src_name);
    var html_im   = on_mouseover_h('im', tgt_name);
    var html      = '<h4 class="mo">Link</h4><p></p>'
                  + html_ib + html_im;
  ...
  //   the sidebar to show consistent colors and text
    ...
    var link_ib  = function(curr_link) {
      var curr_name   = curr_link.target.node.gene;
      var orig_name   = orig_node.node.gene;
    ...
    var link_im  = function(curr_link) {
      var curr_name   = curr_link.source.node.gene;
      var orig_name   = orig_node.node.gene;
    ...
    var node_mo = function(curr_node) {
      var curr_name   = curr_node.node.product;
      var orig_name   = orig_node.node.product;
      var result      = false;
    ...
    var gene      = orig_node.node.gene.split("...")[1].split("_");
    var Toshow="";

    for (i=0;i<gene.length-1;i++){
      Toshow=Toshow+gene[i]+'_';
    }

    Toshow = Toshow.slice(0, - 1);


    var html_ib   = on_mouseover_h('ib', sources);
    var html_im   = on_mouseover_h('im', targets);
    var html      = '<span class="mo">' + '<h4><p>Organism:&nbsp;&nbsp;' + orig_node.node.name + '</p></h4>' + '</span>'
                  + '<span class="mo">' + '<p>Genome Position:&nbsp;&nbsp;' + orig_node.node.genome + '</p>' + '</span>'
                  + '<span class="mo">' + '<p>File Name:&nbsp;&nbsp;' + this.getAttribute("file") + '</p>' + '</span>'
                  + '<span class="mo">' + '<p>Region:&nbsp;&nbsp;' + Toshow + '</p>' + '</span>' + '<span class="mo">' + '<p>Product:&nbsp;&nbsp;' + orig_node.node.product + '</p>' + '</span>'
                  + '<span class="mo">' + '<p>Begin:&nbsp;&nbsp;' + orig_node.node.begin + '</p>' + '</span>'
                  + '<span class="mo">' + '<p>End:&nbsp;&nbsp;' + orig_node.node.end + '</p>' + '</span>' + html_ib + html_im;

    notes.html(html);

    g.svg.append("g").selectAll("text").html(html);

    precis.text(orig_node.node.product);
  }
};
