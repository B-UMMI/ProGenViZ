...
for( i in g.nodesByType){
  ...
  g.svg.graph.append('g')
    ...
    .enter().append('g')
      ...
      .attr('FileName',function(d){return d.FileName;})
      .attr('id', idNode)
...
for(var i=0;i<g.nodesByType.length;i++){
  ...
  for (var y=0;y<lengthtype;y++){
    if (prevBegin!=g.nodesByType[i].values[y].begin){
      ...
      if(g.nodesByType[i].values[y].product!='Undefined'){
       allUndefined='no';
       countNodesGenes+=1;
...
$(function () {
    ...
    $("body").on("contextmenu", "svg g g", function (e) {
        ...
        var Fname=this.getAttribute('FileName');
        ...
        var modalChangePos='';
          ...
          if(SelectionArrayMouse.length>0){
                        ...
                        '<input type="hidden" name="remove_selection"></form></li>';   
                    }
          ...
          insertdrop+= '<hr></hr><div class="modal fade" id="myModalsequenceParameters" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">'+
                                  '<div class="modal-dialog">'+
                                    ...
                                    '<form enctype="multipart/form-data" id="formBLAST" name="formBLAST" action="searchWithContigs.php" method="POST">'+
    ...
    $("div#demo_2").click(function () {
        ...
        if($('#selectionArray').length>0 && SelectionArrayMouse.length == 0) {
