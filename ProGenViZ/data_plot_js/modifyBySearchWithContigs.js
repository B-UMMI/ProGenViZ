...
var display_plot = function(plot_info) {
  ...
  var numberOfTypes= g.nodesByType.length;

  g.angle_dom = [], g.angle_rng = [];

  axes_info = plot_info.axes;
  var generalAngle = 2*Math.PI;
  ...
  for (i in g.nodesByType) {
    ...
    g.angle_dom.push(g.nodesByType[i].key);
    var generalAngle = 2*Math.PI;
    if (i == 0) var TypeAngle = generalAngle;
    else if (i == 1 && numberOfTypes==2) var TypeAngle = generalAngle-((1/3)*generalAngle);
    else var TypeAngle = generalAngle-((i/numberOfTypes)*generalAngle);
    g.angle_rng.push(TypeAngle);
  }
  ...
  var radii     = [ g.inner_radius, g.outer_radius + g.inner_radius ];
...
g.svg.graph = d3.selectAll('g').append('g').attr('transform',g.transform);
  ...
  var x1 = g.radius_f(0) - 200;
  var x2 = function(d) { return g.radius_f(d.count) + 200; };

  var countAxis=0;
  var idAxis = function (d) { countAxis+=1; 
                              return 'Str' + countAxis;};
  ...
  g.svg.graph.selectAll('.axis')
    ...
    .enter().append('line')
      ...
      .attr('id', idAxis)
      .attr('inverso',false)
      .attr('rotacao', function (d){ return g.angle_f(d.key);})
...
var threshold = function(link_node){
  ...
  var transform   = function(i) {
    return 'rotate(' + degrees( g.angle_f(i) ) + ')';
  };
  ...
  g.svg.graph.append('g')
    ...
    .enter().append('g')
      ...
      .enter().append('ellipse')
        .attr('transform', function(d){ return transform3(d.node);})
        .style('stroke', function (d){  
                                        countsEllipse+=1;
                                  ...
                                  '<div class="modal-dialog">'+
                                    '<div class="modal-content">'+
                                      '<div class="modal-header">'+
                                        ...
                                        '<h4 class="modal-title" id="myModalLabel">Choose the E-value and Minimum Score</h4>'+
                                      '</div>'+
                                      '<form enctype="multipart/form-data" action="searchWithContigs.php" method="POST">'+
                                     ...
                                     insert+='<input type="hidden" name="rundatabasesearch" value="rundatabasesearch" />'+
                                          ...
                                          '<label for="E-value">Minimum Score</label>'+
                                          '<input type="name" class="form-control" id="MinIdentity" name="edit_Score" placeholder="0.5">'+
...
$(function () {
    ...
    $("div#SearchTable").click(function () {
        $contextMenu2.hide();
