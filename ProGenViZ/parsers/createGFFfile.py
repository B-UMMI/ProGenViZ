...
try:
	...
	os.chmod(openResultFasta, 0777)
	InputFileSeq=open(openInputSeq, 'r')
	linesIS = InputFileSeq.readlines()
	lastlineIS=len(linesIS)
except IndexError:
	RegionsToExport=genome+'...1---'
	Annotations='no'

toexport=genome+'...'

if toexport not in RegionsToExport:
	RegionsToExport=genome+'...1---'
	Annotations='no'

contigLocations=RegionsToExport.split("---")
...
if Annotations=='yes':
	ToExport='no'
	for j in range(0,len(contigLocations)-1):
		contigToExport=contigLocations[j].split('...')[1]
		match='[Seq'+contigToExport+']'
...
for j in range(0,len(contigLocations)-1):
	...
	for i in range(1,lastlineI):
		if match1 in linesI[i]:
			...
			TocheckRegions=genome+'...'+contigNumber
			if TocheckRegions not in RegionsToExport and firstTimeCheck !='no' and Annotations=='yes':
				firstTimeCheck='no'
				print 'yes'
			elif Annotations=='no' and firstTimeCheck !='no':
				firstTimeCheck='no'
				print 'noAnnotation'
			#print contigNumber
			#print contigToExport
			if contigNumber!=contigToExport and Annotations=='yes':  #MUDAR ISTO ARRANJAR MELHOR SOLUCAO
				match=match
			else:
				try:
